...
public class IdentityOptionsMiddleware
{
    ...
    /// <param name="appOptionsService">The configuration reader.</param>
                                  ...
                                  IOptions<IdentityOptions> identityOptions)
    {
        ...
        if (path.Contains(Login))
            await ApplyNewIdentityOptionsOnLogin(dbContext, appOptionsService, identityOptions);
        else
            await ApplyNewIdentityOptionsOnSpecifiedPaths(dbContext, appOptionsService, identityOptions, path);
                                                                      ...
                                                                      string path)
    {
        ...
        if (pathAllowed)
        {
            ...
            var userSettings = appSettings?.UserSettings ?? appOptionsService.GetAppUserOptions().MapToEntity();
            var passwordSettings = appSettings?.PasswordSettings ?? appOptionsService.GetAppPasswordOptions().MapToEntity();
            var lockoutSettings = appSettings?.LockoutSettings ?? appOptionsService.GetAppLockoutOptions().MapToEntity();
            var signInSettings = appSettings?.SignInSettings ?? appOptionsService.GetAppSignInOptions().MapToEntity();
                                                             ...
                                                             IOptions<IdentityOptions> identityOptions)
    {
        ...
        var lockoutSettings = await dbContext.LockoutSettings.FirstOrDefaultAsync() ?? appOptionsService.GetAppLockoutOptions().MapToEntity();
