...
public class UpdateUserCommand : IRequest<Envelope<string>>
{
    ...
    public class UpdateUserCommandHandler(ApplicationUserManager userManager) : IRequestHandler<UpdateUserCommand, Envelope<string>>
    {
        ...
        public async Task<Envelope<string>> Handle(UpdateUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var concurrencyResult = userManager.CheckForConcurrencyConflict(originalStamp: user.ConcurrencyStamp, currentStamp: request.ConcurrencyStamp);
            ...
            if (!user.IsStatic)
                userManager.AddOrRemoveUserRoles(request.AssignedRoleIds, user);
