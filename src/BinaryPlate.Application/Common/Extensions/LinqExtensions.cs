...
public static class LinqExtensions
{
    ...
    {
        // Ensure pageNumber and totalRowsPerPage are valid values
        pageNumber = pageNumber != 0 ? pageNumber : 1;
        totalRowsPerPage = totalRowsPerPage != 0 ? totalRowsPerPage : 10;

        // Initialize the result PagedList with basic information
        var result = new PagedList<T>
        {
            CurrentPage = pageNumber, // Current page number
            TotalRowsPerPage = totalRowsPerPage,  // Total rows per page
            TotalRows = query.Count()  // Total rows in the query
        };

        // If paging is enabled (totalRowsPerPage != -1)
        if (totalRowsPerPage != -1)
        {
            // Calculate the total number of pages
            var totalPages = (int)Math.Ceiling((double)result.TotalRows / totalRowsPerPage);
            result.TotalPages = totalPages;

            // Calculate the number of items to skip and retrieve items for the current page
            var skip = (pageNumber - 1) * totalRowsPerPage;
            result.Items = await query.Skip(skip).Take(totalRowsPerPage).ToListAsync();
        }
        else
        {
            // If paging is disabled (totalRowsPerPage is -1), retrieve all items in the query
            result.Items = await query.ToListAsync();
        }

        // Return the resulting PagedList
        return result;
    }
    ...
    public static async Task<PagedList<T>> ToPagedListAsync<T>(this IEnumerable<T> query, int pageNumber = 1, int totalRowsPerPage = 10) where T : class
    {
        ...
        var result = new PagedList<T>
        {
            CurrentPage = pageNumber, // Current page number
            TotalRowsPerPage = totalRowsPerPage,  // Total rows per page
            TotalRows = enumerable.Count  // Total rows in the query
        };
        ...
        if (totalRowsPerPage != -1)
        {
            ...
            // Calculate the number of items to skip and retrieve items for the current page.
            var skip = (pageNumber - 1) * totalRowsPerPage;
            result.Items = enumerable.Skip(skip).Take(totalRowsPerPage).ToList();
        }
        else
        {
            // If paging is disabled (totalRowsPerPage is -1), retrieve all items in the query.
            result.Items = enumerable.ToList();
        }
