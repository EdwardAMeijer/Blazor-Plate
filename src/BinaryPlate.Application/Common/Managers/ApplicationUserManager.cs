...
public class ApplicationUserManager(IUserStore<ApplicationUser> store,
                                    ...
                                    IHttpContextAccessor httpContextAccessor) : UserManager<ApplicationUser>(store, optionsAccessor, passwordHasher, userValidators, passwordValidators, keyNormalizer, errors, services, logger)
{
    ...
    public async Task AddOrRemovePermissionsForUserAsync(IReadOnlyList<Guid> selectedPermissionIds, ApplicationUser user)
    {
        ...
        var inheritedUserPermissionIds = (await GetInheritedPermissionsForUserAsync(user.Id, dbContext)).Select(p => p.Id).ToList();
        ...
        RemoveInheritedPermissionsForUserAsync(inheritedUserPermissionIds, selectedPermissionIds, user, dbContext);
    }
    ...
    public async Task<string> SendActivationEmailAsync(ApplicationUser user)
    {
        ...
        // Get client app options and host name.
        var appClientOptions = appOptionsService.GetAppClientOptions();
        var hostName = httpContextAccessor.GetClientAppHostName();

        // Generate callback URL for the activation email.
        var url = $"{hostName}/{appClientOptions.ConfirmEmailUrl}";
    ...
    public async Task<string> SendActivationEmailAsync(ApplicationUser user, string newEmail)
    {
        ...
        if (Options.SignIn.RequireConfirmedAccount)
        {
            ...
            // Generate callback URL for the activation email.
            var url = $"{hostName}/{appClientOptions.ConfirmEmailChangeUrl}";
    ...
    public async Task<string> SendResetPasswordAsync(ApplicationUser user)
    {
        ...
        var url = $"{clientAppOptions.HostName}/{clientAppOptions.ResetPasswordUrl}";
