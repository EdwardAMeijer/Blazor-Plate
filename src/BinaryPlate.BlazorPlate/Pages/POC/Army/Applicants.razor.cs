...
public partial class Applicants : IAsyncDisposable
{
    ...
    [Inject] private IUrlProvider UrlProvider { get; set; }
    ...
    private async Task StartHubConnection()
    {
        if (HubConnection is null || HubConnection.State == HubConnectionState.Disconnected)
        {
            ...
            HubConnection = new HubConnectionBuilder().WithUrl($"{UrlProvider.BaseHubApiUrl}/Hubs/ReportingServicesHub?Accept-Language={CultureInfo.CurrentCulture}",
                                                               options =>
                                                               {
