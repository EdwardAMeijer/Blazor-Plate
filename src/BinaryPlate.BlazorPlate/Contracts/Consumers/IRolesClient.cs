...
public interface IRolesClient
{
    ...
    /// <returns>A <see cref="GetRolePermissionsResponse"/>.</returns>
    Task<ApiResponseWrapper<GetRolePermissionsResponse>> GetRolePermissions(GetRolePermissionsForEditQuery request);

    /// <summary>
    /// Get a list of roles.
    /// </summary>
    /// <param name="request">The query parameters for filtering, sorting and pagination.</param>
    /// <returns>A <see cref="GetRolesResponse"/>.</returns>
    Task<ApiResponseWrapper<GetRolesResponse>> GetRoles(GetRolesQuery request);
