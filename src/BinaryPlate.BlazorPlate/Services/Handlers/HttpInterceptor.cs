...
public class HttpInterceptor : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (response.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden)
        {
            ...
            {
                var relativePath = _navigationManager.ToBaseRelativePath(url);
                var encodedReturnUrl = Uri.EscapeDataString(relativePath);
                _navigationManager.NavigateTo($"/pages/error/401/{encodedReturnUrl}");
            }
        }
